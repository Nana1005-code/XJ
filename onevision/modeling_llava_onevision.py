...
class LlavaOnevisionModel(LlavaOnevisionPreTrainedModel):
    ...
    def get_image_features(
        ...
        """
        获取来自视觉塔的图像最后隐藏状态，并应用多模态投影。

        参数:
            pixel_values (`torch.FloatTensor]`，形状为 `(batch_size, num_patches, channels, height, width)`):
            输入图像对应的张量。
            image_sizes (`torch.Tensor`，形状为 `(num_images, 2)`):
            每张图片的实际尺寸 (高, 宽)。
            vision_feature_layer (`Union[int, List[int]]`):
            选择视觉特征的层索引。如果提供多个索引，则对应层的视觉特征将被拼接以形成最终特征。
            vision_feature_select_strategy (`str`):
            用于从视觉主干中选择视觉特征的策略。可以为 `"default"` 或 `"full"`。
            batch_num_images (`torch.LongTensor`, *可选*):
            每个样本中的图片数量。
        返回:
            image_features (List[`torch.Tensor`]): 图像特征张量列表，每个包含所有 patch 的视觉特征，
            形状为 `(num_patches, image_length, embed_dim)`。
        """
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if pixel_values_videos is not None:
            ...
            '''
            TODO:在这个位置改变嵌入,要注意attention mask要更新
            将input_embeds分开,使用kwargs.get
            计算相似度,对齐,拼接
            得到新的input
            '''
...
class LlavaOnevisionForConditionalGeneration(LlavaOnevisionPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        '''
        TODO:在这里读出某几层的attention,然后加上一个损失
        先读注意力分数,然后分哪个是视觉注意力分数,哪个是文本注意力分数
        新增损失函数
        '''
        loss = None
...
TODO:需要写一个序列对齐模块的类
问题来了:topk怎么定?
这里还得改,因为input_embeds的维度会影响权重矩阵
构造新序列之后仍需要构造一个新的mask来表明什么位置是图像什么位置是文本,我们不需要newline了
更新一个mm_mask
'''
class SequenceAligner(nn.Module):
    ...
    def forward(self, V: torch.Tensor, X: torch.Tensor) -> torch.Tensor:
        #第一步转化维度的不用了，已经转化了，本质这个函数只需要传入文本部分和视觉部分，然后进行计算然后进行拼接即可
        ...
        #拼接序列 [V', X]
        aligned_input = torch.cat([V_aligned, X], dim=0)  # [2 * n2, d2]
        return aligned_input

'''
TODO:将input_embeds拆成文本和视觉的函数
'''
def split_by_token_type(self, inputs_embeds, mm_token_type_ids):
    # 🔧 视觉 vs 文本 token 分割
    text_mask = (mm_token_type_ids == 0).unsqueeze(-1)
    vision_mask = (mm_token_type_ids == 1).unsqueeze(-1)

    text_embeds = inputs_embeds * text_mask
    vision_embeds = inputs_embeds * vision_mask

    text_embeds = text_embeds[text_mask.bool()].view(inputs_embeds.shape[0], -1, inputs_embeds.shape[-1])
    vision_embeds = vision_embeds[vision_mask.bool()].view(inputs_embeds.shape[0], -1, inputs_embeds.shape[-1])

    return vision_embeds, text_embeds

'''
TODO:需要写一个LlavaOnevisionModel的继承类AlignedLlavaOnevisionModel,只需要改变Forward函数
'''
class AlignedLlavaOnevisionModel(LlavaOnevisionModel):
    def __init__(self, config):
        super().__init__(config)
        self.visual_text_aligner = VisualTextAligner(
            #传入文本序列和视觉序列
        )
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        **kwargs,
        **kwargs: Unpack[FlashAttentionKwargs], ##???这个是什么？？
    ) -> Union[Tuple, LlavaOnevisionModelOutputWithPast]:
        ...
        if pixel_values_videos is not None:
            ...
            inputs_embeds = inputs_embeds.masked_scatter(special_video_mask, video_features)
        '''
        此处为分割线,上部分为继承来的forward函数
        '''
        #得到inputs_embeds之后先划分哪里是视觉哪里是文本
        mm_token_type_ids = kwargs.get("mm_token_type_ids", None)

        if mm_token_type_ids is not None:
            vision_embeds, text_embeds = self.split_by_token_type(inputs_embeds, mm_token_type_ids)
            aligned_vision = self.visual_text_aligner(vision_embeds, text_embeds)
            inputs_embeds = torch.cat([aligned_vision, text_embeds], dim=1)

            # 可选：更新 attention_mask
...
TODO:对称熵的函数
'''
def symmetric_kl_divergence(P: torch.Tensor, Q: torch.Tensor, mask: torch.Tensor = None) -> torch.Tensor:
    """
    计算对称 KL 散度:D(P‖Q) + D(Q‖P) / 2
    
    Args:
        P, Q: Tensor of shape [B, ..., dim]，表示两个概率分布（未归一化也可）
        mask: 可选，对 P 和 Q 的有效区域进行掩码（如 [B, ..., 1])
    ...
    kl_PQ = F.kl_div(P_log, Q_soft, reduction='none').sum(dim=-1)  # [B, ...]
    kl_QP = F.kl_div(Q_log, P_soft, reduction='none').sum(dim=-1)
    ...
    if mask is not None:
        ...
        return kl.mean()
'''
TODO:需要写一个LlavaOnevisionForConditionalGeneration的继承类AlignedLlavaOnevisionForConditionalGeneration
    只需要新增一个注意力均衡的损失函数
'''
class AlignedLlavaOnevisionForConditionalGeneration(LlavaOnevisionForConditionalGeneration):
    def __init__(self, config):
        super().__init__(config)
        # 如果你有专门的 attention loss 模块，可以在这里初始化
        # self.attention_loss_fn = MyAttentionLossModule(config)

    '''
    注意力损失计算函数
    '''
    def compute_attention_alignment_loss(self, attentions):
        #这里的attention已经就是具体某一层的注意力输出了
        #根据mask来读取文本和视觉的注意力分数
        mm_token_type_ids = kwargs.get("mm_token_type_ids", None)
        if mm_token_type_ids is not None:
            mm_token_type_ids = torch.tensor(mm_token_type_ids, device=logits.device)
            vision_mask = mm_token_type_ids.bool()        # 图像 token 的 mask
            text_mask = ~vision_mask                      # 文本 token 的 mask

        vision_attn = last_layer_attn[:, :, vision_mask, :]
        text_attn = last_layer_attn[:, :, text_mask, :]

        vision_attn_avg = vision_attn.mean(dim=1)
        text_attn_avg = text_attn.mean(dim=1)

        # -------------------- 局部对齐部分 --------------------
        token_level_loss = symmetric_kl(vision_attn, text_attn)  # P_i vs Q_i

        # -------------------- 全局对齐部分 --------------------
        vision_mean = vision_attn.mean(dim=1)  # [B, L]
        text_mean = text_attn.mean(dim=1)      # [B, L]
        total_level_loss = symmetric_kl(vision_mean, text_mean)
        
        return token_level_loss + total_level_loss
    '''
    多选几层注意力分数来做计算,损失平均
    '''    
    def compute_multi_layer_attention_loss(self, attentions, layers: List[int]):
        total_loss = 0.0
        for layer_idx in layers:
            attn = attentions[layer_idx]
            total_loss += self.compute_attention_alignment_loss(attn)
    ...
    def forward(self, *args, **kwargs):
        ...
        loss = None
        if labels is not None:
            output_loss = self.loss_function(
                logits=logits, labels=labels, vocab_size=self.config.text_config.vocab_size, **kwargs
            )
            loss_attn self.compute_attention_alignment_loss(outputs.attentions)
            λ = getattr(self.config, "attention_loss_weight", 1.0)
            loss = output_loss + λ * loss_attn
