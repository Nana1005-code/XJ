...
class LlavaOnevisionPipeline:
    ...
    def get_text_embeds(self, prompt):
        ...
        text_embeds = text_embeds #[b,18,3584]
    ...
    def expand_image_tokens(self, input_ids, text_embeds):
        ...
        text_token_len = text_embeds.shape[1]
        print(input_ids)
        is_image_token = (input_ids == self.image_token_id)
        print(self.image_token_id)
        print(is_image_token)
        for i in range(len(is_image_token)):
            image_token_index = is_image_token[i].nonzero(as_tuple=True)[0].item()
            print(image_token_index)
            input_ids[image_token_index + 1:]
        repeated_image_tokens = torch.full((text_embeds.shape[0],text_token_len,), self.image_token_id, dtype=torch.long, device=input_ids.device)
        new_input_ids = torch.cat([
            input_ids[:,:image_token_index],
            repeated_image_tokens,
            input_ids[image_token_index + 1:]
    ...
    def get_image_embeds(self, image_path, prompt):
        ...
        print(image_embeds[0].shape,image_embeds[1].shape) #torch.Size([3, 729, 3584]) (num_patches, image_length, embed_dim)
        image_embeds_list=[]
        for image_embed in image_embeds:
            image_embeds = image_embed.reshape(-1, image_embed.size(-1))
