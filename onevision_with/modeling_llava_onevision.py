...
'''
将视觉特征（图像或视频）从视觉编码器的表示空间映射到语言模型的嵌入空间中
以便多模态信息能够被语言模型统一处理
...
class LlavaOnevisionMultiModalProjector(nn.Module):
    def __init__(self, config: LlavaOnevisionConfig):
        ...
        #根据配置参数vision_feature_layer，确定我们要使用哪层的特征值，如果要采用多个层的，就取其长度
        num_feature_layers = 1 if isinstance(config.vision_feature_layer, int) else len(config.vision_feature_layer)
        #线性层1：将视觉特征映射到语言空间
        self.linear_1 = nn.Linear(
            config.vision_config.hidden_size * num_feature_layers, 
            #如果使用多个vision feature layer，则hidden_size * num_feature_layers
            config.text_config.hidden_size,
            bias=config.multimodal_projector_bias,
        )
        self.act = ACT2FN[config.projector_hidden_act] #激活函数
        #线性层2：对已经映射到语言模型空间的向量进一步变换(加上bias的非线性组合)，增强表示能力
...
class LlavaOnevisionModel(LlavaOnevisionPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.vision_tower = AutoModel.from_config(config.vision_config) #视觉编码器

        self.multi_modal_projector = LlavaOnevisionMultiModalProjector(config) #将视觉映射到语言模型的嵌入
        embed_std = 1 / math.sqrt(config.text_config.hidden_size)
        self.image_newline = nn.Parameter(torch.randn(config.text_config.hidden_size, dtype=self.dtype) * embed_std) #imgae_newline是图像结束后的换行符。在多图或者多patch处理中使用，作为图像之间的分隔符

        self.vocab_size = config.text_config.vocab_size
        self.language_model = AutoModel.from_config(config.text_config) #主体模型，看text_config是Qwen2-7B-Instruct
        self.pad_token_id = self.config.pad_token_id if self.config.pad_token_id is not None else -1
        self.post_init()

    def get_input_embeddings(self):
        return self.language_model.get_input_embeddings() #返回language model的embedding

    def set_input_embeddings(self, value): #新的embedding层，可用于替换默认的embedding层
        self.language_model.set_input_embeddings(value)

    def pack_image_features(self, image_features, image_sizes, image_newline=None, vision_aspect_ratio="anyres_max_9"):
        """
        这个方法负责 将图像塔输出的 patch 嵌入序列进行整理和 reshape 用于与文本嵌入拼接或融合。
        将图像特征从视觉编码器中输出的形式转化为适合语言模型输出的形式
        Reshape, unpad and then pack each image_feature into a single image_features tensor containing all visual vectors.

        Args:
            图像patch嵌入列表:image_features (`List[torch.Tensor]` of length num_images, each of shape `(num_patches, image_length, embed_dim)`)
                List of image feature tensor, each contains all the visual feature of all patches.
            每张图像的实际尺寸:image_sizes (`torch.Tensor` of shape `(num_images, 2)`)
                Actual image size of each images (H, W).
            图像换行符:image_newline (`torch.Tensor` of shape `(embed_dim)`)
                New line embedding vector.
            控制最大的patch数量:vision_aspect_ratio (`str`, *optional*, "anyres_max_9"):
        ...
        """
        new_image_features = [] #存储处理后的图像特征
        feature_lens = [] #存储每张图像对应的token数量
        
        #遍历每张图片进行处理
        for image_idx, image_feature in enumerate(image_features):
            #多patch图像处理
            if image_feature.shape[0] > 1: 
                base_image_feature = image_feature[0] #第一个patch通常是CLS token，作为图像的基础表示保留下来
                image_feature = image_feature[1:] #除去第一个patch，其他patch是图像不同区域的特征
                
                #这里的height和weight代表在行列上有多少patch
                height = width = self.config.vision_config.image_size // self.config.vision_config.patch_size 
                if height * width != base_image_feature.shape[0]: ##？？？？？？？
                    raise ValueError("The number of patches is not consistent with the image size.")
                
                #动态调整 patch 网格数量以适应任意分辨率
                num_patch_height, num_patch_width = get_anyres_image_grid_shape( # 动态计算适应任意分辨率的 patch 网格大小。
                ...
                #如果图像的 patch 数量大于最大允许值，则进行插值缩放以适应最大值。
                ...
                #在图像末尾(不是patch末尾)插入 image_newline 向量，作为图像之间的分隔符。
    ...
    def get_image_features(
        ...
        """
        从视觉编码器中提取图像特征,并进行多模态投影,返回适合语言模型使用的图像patch嵌入。
        之后要拿给pack_image_features对输出图像进行进一步后处理,使其适配语言模型的输入格式
        Obtains image last hidden states from the vision tower and apply multimodal projection.

        Args:
            图像像素值张量:pixel_values (`torch.FloatTensor]` of shape `(batch_size, num_patches, channels, height, width)`)
               The tensors corresponding to the input images.
            图像实际尺寸:image_sizes (`torch.Tensor` of shape `(num_images, 2)`)
                Actual image size of each images (H, W).
            指定从哪个视觉编码层提取特征,可以是多个层索引:vision_feature_layer (`Union[int, List[int]]`):
                The index of the layer to select the vision feature. If multiple indices are provided,
                the vision feature of the corresponding indices will be concatenated to form the
                vision features.
            特征选择策略:vision_feature_select_strategy (`str`):
                The feature selection strategy used to select the vision feature from the vision backbone.
                Can be one of `"default"`去除CLS or `"full"`保留
            每个样本中的图像数量:batch_num_images (`torch.LongTensor`, *optional*):
        ...
        #根据每张图的输入尺寸 image_sizes 
        #和是否需要进行 patch 分割（由 batch_num_images 控制）来计算每张图应该有多少个 patch。
        if batch_num_images is None: #每个batch中包含多少图像 单图像模式
        ...
        #将多个样本的的patch合并成一个4D张量[total_patches, channels, height, width]
        #将一个batch的多个patch拼接成一个张量(选择固定的patch数)
        if pixel_values.dim() == 5: #(batch_size, num_patches, channels, height, width)
            # stacked if input is (batch_size, num_patches, num_channels, height, width)
            _pixel_values_list = [pix_val[:num_patch] for pix_val, num_patch in zip(pixel_values, image_num_patches)]
            pixel_values = torch.cat(_pixel_values_list, dim=0)
        elif pixel_values.dim() != 4: #(num_patches, channels, height, width)
        ...
        #从视觉编码器输出的隐藏状态中选择特定层，然后将其组合成最终的图像特征表示
        #如果只指定了一个视觉特征层，就直接返回该层的隐藏状态
        #如果指定了多个层，则提取这些层的隐藏状态并按特征维度拼接起来。
        ...
        #如果使用default策略，就要去掉CLS特征，如果使用full，就要保留所有特征【看自己需求】
        ...
        ####################################################################
        #使用 multi_modal_projector 对选定的特征进行多模态投影
        #将视觉编码器输出的图像特征映射到语言模型的嵌入空间中，使得图像特征能够与文本嵌入进行融合和交互。
        image_features = self.multi_modal_projector(selected_image_feature)
        image_features = torch.split(image_features, image_num_patches, dim=0)

        #调用 pack_image_features 方法将图像特征整理成适合语言模型使用的格式。
        #返回处理后的图像特征。
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        #输入校验???目的是什么
        #检查是否 只传入了 input_ids 或 inputs_embeds 中的一个
        if (input_ids is None) ^ (inputs_embeds is not None):
            raise ValueError("You must specify exactly one of input_ids or inputs_embeds")
        #传入视频或者图片 并且存在inputs_embeds inputs_embeds就是文本嵌入
        ...
        if pixel_values is not None: #是否有图像输入
            image_features = self.get_image_features( #获取图像特征
                pixel_values,
                image_sizes,
            ...
            image_features = torch.cat(image_features, dim=0) #将每张图像的 patch 特征拼接成一个大的 tensor
...
'''
该类是对 LlavaOnevisionModel 的封装，用于条件生成任务（如图文问答）。其中也直接暴露了语言模型部分
这一部分之前忘记看了 一定要看
'''
